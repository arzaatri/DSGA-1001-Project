...
def get_scores(pipe, train_X, test_X, train_y, test_y, scores):
    ...
    preds_v = pipe.predict(train_X)
    proba_v = pipe.predict_proba(train_X)[:,1]
    preds_t = pipe.predict(test_X)
    proba_t = pipe.predict_proba(test_X)[:,1]
    for s in scores:
        if s in ['roc_auc_score', 'average_precition_score']:
            model_scores['validation_'+s] = eval('metrics.'+s+'(train_y, proba_v)')
            model_scores['test_'+s] = eval('metrics.'+s+'(test_y, proba_t)')
        else:
            model_scores['validation_'+s] = eval('metrics.'+s+'(train_y, preds_v)')
            model_scores['test_'+s] = eval('metrics.'+s+'(test_y, preds_t)')
